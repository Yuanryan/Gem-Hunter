...

{
...

    {
        ...
        private const float MAX_DRAG_TIME = 5.0f;
        private bool m_TimerStarted = false; // 追蹤計時器是否已開始
        ...
        void UpdateDragging(Vector3 worldPos)
        {
            ...
            if (m_TimerStarted)
            {
                float dragTime = Time.time - m_DragStartTime;
                float remainingTime = MAX_DRAG_TIME - dragTime;
                
                // 更新時間條
                UIHandler.Instance.UpdateDragTimer(remainingTime, MAX_DRAG_TIME);
                
                if (dragTime >= MAX_DRAG_TIME)
                {
                    // 時間到了，強制結束拖拽
                    EndDragging();
                    return;
        ...
        void EndDragging()
        {
            ...
            if (m_TimerStarted)
            {
                UIHandler.Instance.UpdateDragTimer(MAX_DRAG_TIME, MAX_DRAG_TIME);
                m_TimerStarted = false;
                Debug.Log("拖拽結束，隱藏時間條");
            }
        ...
        void PerformRealtimeSwap(Vector3Int fromCell, Vector3Int toCell)
        {
            ...
            if (!m_TimerStarted)
            {
                ...
                UIHandler.Instance.ShowDragTimer();
                Debug.Log("第一次交換，開始計時");
            }
